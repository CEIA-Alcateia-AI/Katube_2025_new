...
class AudioProcessingPipeline:
    ...
    def prepare_for_stt(self, separation_results: Dict[str, Any]) -> Dict[str, List[Path]]:
        ...
        for audio_result in separation_results.values():
            if 'error' in audio_result:
                print("\n\n\\n =============== ERRO NA SEPARACAO DE FALANTES, PULANDO ==============\n\n")
                continue
    ...
    def transcribe_audio_segments(self, 
        ...
        try:
            ...
            # Step 8: Validate STT transcriptions with Levenshtein
            ...
            if normalization_result.get('success'):
                try:
                    ...
                    if normalized_json_path:
                        # Executar validacao (Levenshtein)
                ...
                logger.warning("Normalization failed, validating without normalization")
                combined_results['validation'] = {"error": "Normalization failed"}
            
            # Step 9: Filter by similarity threshold, then apply denoising
    ...
    def process_local_audio(self,  
        ...
        if not ogg_files:
            error_msg = f"Nenhum arquivo .ogg encontrado no diretório: {audio_path}"
            logger.error(f"❌ {error_msg}")
            raise FileNotFoundError(error_msg)
        source_audio_path = ogg_files[0]

        video_id = audio_path.name # Assume que o nome da pasta é o ID
        session_name_resolved = session_name or video_id

        try:
            # Create session
            session_dir = self.create_session(session_name_resolved)
            
            # 2. Define um nome CURTO para o arquivo de trabalho
            safe_filename = "audio.flac" 
            initial_audio_in_session = session_dir / 'downloads' /safe_filename
            
            # 3. Copia e Converte/Normaliza
            # O normalizer vai ler o OGG original e salvar como "audio_source.flac"
            logger.info(f"🔄 Normalizing {source_audio_path.name} -> {safe_filename}")
            
            normalization_result = self.audio_normalizer.normalize_audio(
                input_path=source_audio_path, 
                output_path=initial_audio_in_session
            )
            
            if normalization_result['success']:
                audio_path_for_pipeline = initial_audio_in_session
                logger.info(f"✅ Audio ready: {audio_path_for_pipeline}")
            else:
                raise RuntimeError(f"Normalization failed: {normalization_result.get('error')}")
            ...
            # Step 7: Move approved segments to final directory
            try:
    ...
    def filter_and_denoise_segments(self, 
                                   ...
                                   output_dir: Path):
        """
        Step 9: Filter segments by similarity, apply denoising when needed.
        ...
        try:
            ...
            for segment_id, pair_data in normalized_pairs.items():
                ...
                if not audio_file or not audio_file.exists():
                    segments_aprovados_dir = output_dir / 'segments'
                    if segments_aprovados_dir.exists():
                        for flac_path in segments_aprovados_dir.glob("*.flac"):
                            prefix = segment_id.split('_stt_')[0] if '_stt_' in segment_id else segment_id
                            if prefix in flac_path.stem or (flac_file and flac_file in flac_path.name):
                                audio_file = flac_path
                                break
                
                # PASSAR DENOISER NOS SEGMENTOS DE ÁUDIO 
                logger.warning(f"  Passando Denoiser nos áudios de baixa qualidade...")
                status = 'approved'
            ...
            approved_total = approved_with_denoise
        ...
        try:
            ...
            for segment_id, seg_data in approved_segments.items():
                ...
                if not audio_file.exists():
                    logger.warning(f"⚠️ Áudio final não encontrado para {segment_id}. Pulando este registro no CSV.")
                    continue # Pula para o próximo segmento do loop
