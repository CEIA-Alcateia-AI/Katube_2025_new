...
def validar_pasta_audios(id_video: str, total_segmentos: int) -> tuple[bool, str]:
    """
    Valida se pasta de audios do dataset existe e nao esta vazia
    
    Args:
        id_video: ID do video
        total_segmentos: Quantidade de segmentos esperados no JSON
        
    Returns:
        Tupla (valido, mensagem_erro)
        - valido: True se passou em todas validacoes, False caso contrario
        - mensagem_erro: String vazia se valido, mensagem de erro caso contrario
    """
    pasta_audios = PROJECT_ROOT / "dataset" / "audio_dataset" / id_video
    
    # Validacao 1: Pasta existe?
    if not pasta_audios.exists():
        return False, f"ERRO: Pasta de audios nao existe: {pasta_audios}"
    
    # Validacao 2: E um diretorio?
    if not pasta_audios.is_dir():
        return False, f"ERRO: Caminho nao e um diretorio: {pasta_audios}"
    
    # Validacao 3: Pasta tem arquivos?
    arquivos = list(pasta_audios.iterdir())
    if len(arquivos) == 0:
        return False, f"ERRO: Pasta de audios esta vazia: {pasta_audios}"
    
    # Validacao 4: Quantidade de arquivos vs segmentos (apenas aviso)
    if len(arquivos) != total_segmentos:
        print(f"AVISO: Quantidade de arquivos ({len(arquivos)}) diferente de segmentos no JSON ({total_segmentos})")
        print(f"Pasta: {pasta_audios}")
...
def processar_metadados(id_video: str):
    """
    ...
    # Validar pasta de audios antes de processar
    valido, mensagem_erro = validar_pasta_audios(id_video, len(dados_json))
    if not valido:
        print(mensagem_erro)
        print("Processamento abortado: pasta de audios invalida ou vazia")
        print("-" * 80)
        return
    
    print("Validacao de pasta de audios: OK")
    print("-" * 80)
...
if __name__ == "__main__":
    processar_metadados('B4RgpqJhoIo')